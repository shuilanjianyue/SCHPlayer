...
@interface SCHPlayerView ()<SCHPlayerMaskViewDelegate,UIGestureRecognizerDelegate>
...
/**遮罩*/
@property (nonatomic, strong) SCHPlayerMaskView *maskView;
/** 用来保存快进的总时长 */
...
@implementation SCHPlayerView



#pragma mark - 懒加载
//遮罩
- (SCHPlayerMaskView *) maskView{
    if (_maskView == nil){
        _maskView                         = [[SCHPlayerMaskView alloc] init];
        ...
        //计时器，循环执行
        [[SCHGCDTimerManager sharedManager] scheduledDispatchTimerWithName:CLPlayer_sliderTimer
                                                             timeInterval:1.0f
                                                                delaySecs:0
        ...
        [[SCHGCDTimerManager sharedManager] startTimer:CLPlayer_sliderTimer];
    }
    return _maskView;
}
...
-(void)setFullStatusBarHiddenType:(FullStatusBarHiddenType)fullStatusBarHiddenType{
    ...
    switch (fullStatusBarHiddenType) {
        ...
        case FullStatusBarHiddenFollowToolBar:
            ...
            break;
        case FullStatusBarHiddenWhenFull:
            if (_isFullScreen) {
                [self setStatusBarHidden:YES];
            }else{
                [self setStatusBarHidden:NO];
...
-(void)setToolBarDisappearTime:(NSInteger)toolBarDisappearTime{
    ...
    //定时器，工具条消失
    [[SCHGCDTimerManager sharedManager] scheduledDispatchTimerWithName:CLPlayer_tapTimer
    ...
    [[SCHGCDTimerManager sharedManager] startTimer:CLPlayer_tapTimer];
}
#pragma mark - 传入播放地址
...
- (void)panDirection:(UIPanGestureRecognizer *)pan {
    ...
    switch (pan.state) {
        case UIGestureRecognizerStateBegan:{ // 开始移动
            ...
            [self sch_progressSliderTouchBegan:nil];
            //显示遮罩
        ...
        case UIGestureRecognizerStateEnded:{ // 移动停止
            // 把sumTime滞空，不然会越加越多
            self.sumTime = 0;
            [self sch_progressSliderTouchEnded:nil];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    ...
    if ([touch.view isDescendantOfView:self.maskView.bottomToolBar]) {
        ...
        if (!_isFullScreen) {
            ...
            return _smallGestureControl;
        }else{
            
            return _fullGestureControl;
        }
    }
...
- (void)pausePlay{
    self.maskView.playButton.selected = NO;
    [_player pause];
    [[SCHGCDTimerManager sharedManager] suspendTimer:CLPlayer_sliderTimer];
}
#pragma mark - 播放
- (void)playVideo{
    _isUserPlay                       = YES;
    ...
    if (_isEnd && self.maskView.slider.value == 1) {
        ...
        [_player play];
        [[SCHGCDTimerManager sharedManager] resumeTimer:CLPlayer_sliderTimer];
    }
}
#pragma mark - 重新开始播放
- (void)resetPlay{
...
- (void)destroyAllTimer{
    [[SCHGCDTimerManager sharedManager] cancelTimerWithName:CLPlayer_sliderTimer];
    [[SCHGCDTimerManager sharedManager] cancelTimerWithName:CLPlayer_tapTimer];
}
//销毁定时消失定时器
...
- (void)fullScreenWithDirection:(UIInterfaceOrientation)direction{
    ...
    self.fullStatusBarHiddenType = _fullStatusBarHiddenType;
