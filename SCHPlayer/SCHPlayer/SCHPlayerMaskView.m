...
@implementation SCHPlayerMaskView
...
//转子
- (SCHLoadingView *)activity{
    if (_activity == nil){
        _activity             = [[SCHLoadingView alloc] initWithFrame:CGRectMake(0, 0, 30, 30)];
...
- (void)backButtonAction:(UIButton *)button{
    if (_delegate && [_delegate respondsToSelector:@selector(sch_backButtonAction:)]) {
        [_delegate sch_backButtonAction:button];
    }else{
...
- (void)playButtonAction:(UIButton *)button{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(sch_playButtonAction:)]) {
        [_delegate sch_playButtonAction:button];
    }else{
        NSLog(@"没有实现代理或者没有设置代理人");
    }
}
...
- (void)fullButtonAction:(UIButton *)button{
    button.selected = !button.selected;
    if (_delegate && [_delegate respondsToSelector:@selector(sch_fullButtonAction:)]) {
        [_delegate sch_fullButtonAction:button];
    }else{
        NSLog(@"没有实现代理或者没有设置代理人");
    }
}
...
- (void)failButtonAction:(UIButton *)button{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(sch_failButtonAction:)]) {
        [_delegate sch_failButtonAction:button];
    }else{
        NSLog(@"没有实现代理或者没有设置代理人");
    }
}
#pragma mark - 滑杆
//开始滑动
- (void)progressSliderTouchBegan:(SCHSlider *)slider{
    if (_delegate && [_delegate respondsToSelector:@selector(sch_progressSliderTouchBegan:)]) {
        [_delegate sch_progressSliderTouchBegan:slider];
    }else{
        NSLog(@"没有实现代理或者没有设置代理人");
    }
}
//滑动中
- (void)progressSliderValueChanged:(SCHSlider *)slider{
    if (_delegate && [_delegate respondsToSelector:@selector(sch_progressSliderValueChanged:)]) {
        [_delegate sch_progressSliderValueChanged:slider];
    }else{
        NSLog(@"没有实现代理或者没有设置代理人");
    }
}
//滑动结束
- (void)progressSliderTouchEnded:(SCHSlider *)slider{
    if (_delegate && [_delegate respondsToSelector:@selector(sch_progressSliderTouchEnded:)]) {
        [_delegate sch_progressSliderTouchEnded:slider];
    }else{
        NSLog(@"没有实现代理或者没有设置代理人");
    }
}
