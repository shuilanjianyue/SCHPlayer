...
@implementation SCHGCDTimerManager

#pragma mark - 初始化
+ (instancetype)sharedManager {
    static SCHGCDTimerManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[SCHGCDTimerManager alloc] init];
    });
    return manager;
}
- (instancetype)init {
...
- (void)adddDispatchTimerWithName:(NSString *)timerName
    ...
    SCHGCDTimer *GCDTimer = self.timerObjectCache[timerName];
    if (!GCDTimer) {
        GCDTimer = [[SCHGCDTimer alloc] initDispatchTimerWithName:timerName
